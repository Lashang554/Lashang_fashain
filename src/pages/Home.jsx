...
const Home = () => {
  const { data, fetchAllProducts, categoryOnlyData, loading } = useContext(DataContext) || {}

  useEffect(() => {
    if (fetchAllProducts && (!data || data.length === 0)) {
      fetchAllProducts()
    }
  }, [fetchAllProducts, data])

  const topCategories = Array.isArray(categoryOnlyData) ? categoryOnlyData.slice(1, 7) : []
  const featured = Array.isArray(data) ? data.slice(0, 8) : []
  const deals = Array.isArray(data) 
    ? [...data].filter(p => (typeof p.discount === 'number' ? p.discount : parseFloat(p.discount) || 0) > 0)
    ...
    <div className='overflow-x-hidden'>
      ...
      <section className='max-w-7xl mx-auto px-4 mt-12'>
        <div className='flex items-center justify-between'>
          ...
          <Link to='/products?q=%25' className='text-[#F85606] font-semibold'>View all</Link>
        </div>
        {loading ? (
